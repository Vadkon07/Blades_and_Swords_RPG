...
So, we started to make our game GUI based, yes. For GUI we use GTK, and I think that probably it will be more than enough. The latest versions of code which are published here - that's a broken codes, just as a drafts. If you want - you can help us to add a GUI in our game, because it's not easy to port it all alone. All our tasks you can find below in a To-Do list.

# Moonshadow RPG
**Moonshadow RPG** is a text-based RPG game. In this game, you can engage in fights, eat food to heal your character, earn money, level up, and more.
...
## How to Play
1. **Starting the Game:**
- Compile a game by entering **gcc -o game main.c `pkg-config --cflags --libs gtk+-3.0`**.
...
```console
git clone https://github.com/Vadkon07/Moonshadow.git
cd Moonshadow/
```
2. **Compile the code:**
```console
gcc -o moonshadow main.c `pkg-config --cflags --libs gtk+-3.0`
```
3. **Run the game:**
```console
./moonshadow
...
- [ ] **Enhance Graphics and GUI:** Upgrade the game’s visual presentation and user interface for a better experience. At first we should fully transfer out game to make it GUI based!
...
- [ ] **Create a Story:** Story, places, characters. Create our own world.
